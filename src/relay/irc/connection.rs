...
fn parse_names_entry(entry: &str) -> Nick {
    ...
    Nick { name: name.to_string(), prefix: prefix.to_string(), color_ansi: nick_color_ansi(name) }
}
    ...
    match msg.command.as_str() {
            ...
            match sub.as_str() {
                "LS" => {
                    ...
                    if !is_continuation {
                        ...
                        if !req.is_empty() {
                            session.whois_lines.push(format!("CAP REQ :{}", req)); // reuse field as write queue — see caller
                        } else {
        ...
        "JOIN" => {
            ...
            if nick.eq_ignore_ascii_case(&session.our_nick) {
                ...
                let new_nick = Nick { name: nick.clone(), prefix: String::new(), color_ansi: nick_color_ansi(&nick) };
                events.push(BackendEvent::NickAdded { buffer_id: chan_lower.clone(), nick: new_nick });
                *line_counter += 1;
                let line = make_line(&line_counter.to_string(), "-->", &format!("{} has joined {}", nick, channel), timestamp_from_msg(msg), false);
        ...
        "NICK" => {
            ...
            for chan in session.joined.clone() {
                ...
                events.push(BackendEvent::NickAdded { buffer_id: chan.clone(), nick: Nick { name: new_nick.clone(), prefix: String::new(), color_ansi: nick_color_ansi(&new_nick) } });
                *line_counter += 1;
        ...
        "352" => {
            ...
            if let Some(existing) = bucket.iter_mut().find(|n| n.name == nick) {
                ...
                bucket.push(Nick { name: nick.clone(), prefix: prefix.to_string(), color_ansi: nick_color_ansi(&nick) });
            }
        }
